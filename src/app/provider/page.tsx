...
export default function ProviderDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold outfit">{t("provider.hello")} {session?.user?.name}</h1>
        <p className="text-muted-foreground mt-1">{t("provider.myRequests")}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-card p-5 rounded-2xl border-white/5">
          ...
          <div className="text-sm text-muted-foreground">{t("provider.awaitingStart")}</div>
        </div>
        <div className="glass-card p-5 rounded-2xl border-white/5">
          <div className="text-3xl font-bold text-purple-400">{inProgress.length}</div>
          <div className="text-sm text-muted-foreground">{t("provider.inProgress")}</div>
        </div>
        <div className="glass-card p-5 rounded-2xl border-white/5">
          <div className="text-3xl font-bold text-green-400">{completed.length}</div>
          <div className="text-sm text-muted-foreground">{t("provider.completed")}</div>
        </div>
      </div>
      ...
      {!user?.verified && (
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 flex gap-3 text-sm text-amber-200">
          ...
          <div>
            <p className="font-bold mb-1">{t("provider.accountUnderReview")}</p>
            <p className="opacity-80">{t("provider.accountUnderReviewDesc")}</p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="glass-card p-16 text-center rounded-3xl border-white/5">
          ...
          <h3 className="text-xl font-bold mb-2">{t("provider.noRequests")}</h3>
          <p className="text-muted-foreground">{t("provider.noRequestsDesc")}</p>
        </div>
      ) : (
        <div className="space-y-4">
          {requests.map((req, i) => (
                ...
                <div className="flex gap-4 items-start flex-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      {req.property?.location || t("provider.addressNotAvailable")}
                    </div>
                  ...
                  {req.scheduledDate && (
                    <div className="flex flex-col items-end bg-white/5 px-3 py-2 rounded-xl">
                      <div className="text-[10px] text-amber-500 flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {t("request.inspectionDate")}
                      </div>
